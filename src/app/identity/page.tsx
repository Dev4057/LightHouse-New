...
import { ShieldAlert, Clock, ShieldCheck, UserCheck, AlertTriangle, Loader, Search, Filter, Shield } from 'lucide-react'
...
export default function IdentityPage() {
  ...
  const summary = useMemo(() => {
    ...
    return { NO_MFA_COUNT: raw?.NO_MFA_COUNT ?? 0, DORMANT_COUNT: raw?.DORMANT_COUNT ?? 0, ADMIN_COUNT: raw?.ADMIN_COUNT ?? 0 };
  }, [summaryRes]);
  ...
  const filteredUsers = useMemo(() => {
    ...
    return displayUsers.filter((user: any) => {
      ...
      return ((user.USER_NAME || '').toLowerCase().includes(query) || (user.TYPE || '').toLowerCase().includes(query) || roleText.includes(query));
    });
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6 animate-in fade-in duration-500">
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <InfoTooltip text="This is your security checkpoint. It lists everyone who has the keys to your database. Red tags mean their account lacks Multi-Factor Authentication (MFA) and is vulnerable to hackers." />
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <StatCard label="Users Without MFA" value={summary.NO_MFA_COUNT} desc="Critical security gap" icon={<ShieldAlert className="w-5 h-5 text-red-600 dark:text-red-400"/>} iconBg="bg-red-100 dark:bg-red-900/30" />
          <StatCard label="Dormant Accounts" value={summary.DORMANT_COUNT} desc="Inactive 90+ days" icon={<Clock className="w-5 h-5 text-amber-600 dark:text-amber-400"/>} iconBg="bg-amber-100 dark:bg-amber-900/30" />
          <StatCard label="Account Admins" value={summary.ADMIN_COUNT || adminNames.length || 1} desc="Highly privileged identities" icon={<Shield className="w-5 h-5 text-blue-600 dark:text-blue-400"/>} iconBg="bg-blue-100 dark:bg-blue-900/30" />
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm">
          <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row justify-between items-center gap-4 bg-slate-50/50 dark:bg-slate-800/50">
            <div className="relative w-full sm:max-w-xs">
              ...
              <input type="text" placeholder="Search by name or role..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-9 pr-4 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none transition-all" />
            </div>
                      ...
                      <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${isAdmin ? 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400' : 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300'}`}>
                        ...
                        <td className="px-6 py-4">
                          <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium border ${isAdmin ? 'bg-purple-50 text-purple-700 border-purple-200 dark:bg-purple-900/20 dark:border-purple-800 dark:text-purple-400' : 'bg-slate-50 text-slate-700 border-slate-200 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-300'}`}>
                        ...
                        <td className="px-6 py-4 text-right">
                          <button onClick={() => setSelectedUser(user)} className="px-3 py-1.5 bg-slate-100 hover:bg-blue-50 text-slate-600 hover:text-blue-600 rounded-lg text-xs font-bold transition-all">Review</button>
                        </td>
                      </tr>
                    )
                  })
                ) : (
                  <tr><td colSpan={5} className="px-6 py-12 text-center"><p className="text-sm text-slate-500 dark:text-slate-400">No users found matching "{searchQuery}"</p></td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      {selectedUser && <IdentityReviewPanel user={selectedUser} onClose={() => setSelectedUser(null)} onRefresh={() => window.location.reload()} />}
    </DashboardLayout>
  )
}
