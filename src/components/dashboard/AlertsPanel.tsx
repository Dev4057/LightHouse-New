...
export default function AlertsPanel({ dateRange }: { dateRange: { start: Date; end: Date } }) {
  ...
  if (kpi) {
    ...
    if (avgTime > 0.01) { // Changed from 2s to 0.01s
      alerts.push({
        id: 'avg-time',
        title: 'High avg. query time',
        severity: avgTime > 1 ? 'critical' : 'warning',
    ...
    // 🚀 LOWERED THRESHOLDS FOR TESTING
    if (failed > -1) { // Changed from > 0 to > -1 (Forces it to show even if 0)
      alerts.push({
        id: 'failed-q',
    ...
    // 🚀 LOWERED THRESHOLDS FOR TESTING
    if (costPerDay >= 0) { // Changed from > 100 to >= 0
      alerts.push({
        id: 'cost-day',
        title: `${creditUnitLabel} above baseline`,
        severity: costPerDay > 10 ? 'critical' : 'warning',
  ...
  // 🚀 REMOVED STRICT LENGTH CHECK FOR TESTING
  if (idle) { 
    const topIdle = idle[0] || { WAREHOUSE_NAME: 'Test_WH', idle_credits: 2.5 } // Fake data fallback
    alerts.push({
      id: 'idle-cost',
      title: 'Idle warehouse detected',
      severity: convertCredits(Number(topIdle?.idle_credits || 0)) > (creditUnitLabel === 'USD' ? 5 : 1) ? 'critical' : 'warning',
  ...
  return (
    <div className="
      ...
      shadow-sm dark:shadow-xl transition-all duration-300
    ">

      {/* ── HEADER ── */}
      <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200 dark:border-slate-700/50 bg-slate-50/50 dark:bg-slate-800/20">
        <div className="flex items-center gap-2.5">
          <div className="relative flex h-5 w-5 items-center justify-center shrink-0">
            ...
            <Activity className={`relative z-10 w-4 h-4 ${criticalCount > 0 ? 'text-red-500' : 'text-blue-500'}`} />
          </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
        {alerts.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-40 text-center">
            <div className="w-12 h-12 rounded-full bg-emerald-50 dark:bg-emerald-500/10 flex items-center justify-center mb-3 border border-emerald-100 dark:border-emerald-500/20">
              <ShieldAlert className="w-6 h-6 text-emerald-500 dark:text-emerald-400" />
            </div>
            <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">All Systems Normal</p>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">No anomalies detected in this window.</p>
          </div>
        ) : (
          alerts.map((alert) => {
            ...
            return (
              <div
                ...
                className={`
                  ...
                  ${isCritical
                    ? 'border-l-red-500 bg-red-50/50 dark:bg-red-950/20 border border-red-100 dark:border-red-900/30'
                    : 'border-l-amber-500 bg-amber-50/50 dark:bg-amber-950/20 border border-amber-100 dark:border-amber-900/30'
                  }
                `}
              >
                {/* Icon */}
                ...
                <div className="flex-1 min-w-0 pt-0.5">
                  <p className={`text-[13px] font-bold leading-snug truncate
                    ${isCritical
                      ? 'text-red-900 dark:text-slate-100'
                      : 'text-amber-900 dark:text-slate-100'
                    }`}>
                    {alert.title}
                  </p>
                  <p className={`text-xs mt-1 leading-snug truncate
                    ${isCritical
                ...
                <div className="shrink-0 flex items-center gap-1 text-slate-500 dark:text-slate-400 mt-1">
